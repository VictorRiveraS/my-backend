...
class CategoriesService {
    ...
    async fetchSubcategoriesService(body, page, limit, skip) {
        try {
            let subcategories;
            if (page == 0) {
                subcategories = await categories_model_1.subcategoriesModel.find(body);
            }
            else {
                subcategories = await categories_model_1.subcategoriesModel.find(body).setOptions({ skip: skip, limit: limit });
            }
            if (!subcategories) {
                return [400, {
    ...
    async fetchSubsubcategoriesService(body, page, limit, skip) {
        try {
            ...
            if (page == 0) {
                subsubcategories = await categories_model_1.subsubcategoriesModel.find(body);
            }
            else {
                subsubcategories = await categories_model_1.subsubcategoriesModel.find(body).setOptions({ skip: skip, limit: limit });
            }
            if (!subsubcategories) {
                return [400, {
